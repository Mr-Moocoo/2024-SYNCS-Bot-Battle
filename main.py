...
def handle_claim_territory(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    priority = [41, 38, 40, 39]
    unclaimed_territories = game.state.get_territories_owned_by(None)
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)
    
    # First try to claim a territory from the priority list
    for i in priority:
        if i in unclaimed_territories:
            return game.move_claim_territory(query, i)
    
    # If no territories from the priority list are available, try to claim adjacent territories
    ...
    if available:
        ...
        # If there are no available adjacent territories, pick any unclaimed territory with the greatest degree
...
def handle_place_initial_troop(game: Game, bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    priority = [40, 39, 41, 38]
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)
    
    # Check if any priority territory is available and return the first one found
    for territory_id in priority:
        if territory_id in my_territories:
            return game.move_place_initial_troop(query, territory_id)
    
    # If no priority territory is available, find the border territory with minimum troops
...
def handle_redeem_cards(game: Game, bot_state: BotState, query: QueryRedeemCards) -> MoveRedeemCards:
    ...
    # We will always redeem the minimum number of card sets we can until the 6th card set has been redeemed.
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    priority = [41, 38, 40, 39, 30, 29, 31, 28]
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)
    ...
    # Prioritize attacking territories from the priority list
    priority_targets = list(set(priority) & set(bordering_territories))
    move = attack_weakest(priority_targets)
    if move:
        return move
...
def handle_fortify(game: Game, bot_state: BotState, query: QueryFortify) -> Union[MoveFortify, MoveFortifyPass]:
    ...
    # We will always fortify towards the weakest player (player with the fewest troops on the map) to strengthen against them.
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)
    
    # Calculate total troops per player
    ...
    # Find the weakest player by total troops
    weakest_player = min(total_troops_per_player.items(), key=lambda x: x[1])[0]
    
    # If we are the weakest player, we will pass.
    if weakest_player == game.state.me.player_id:
        return game.move_fortify_pass(query)
    
    # Find our territory with the most troops
    ...
    # Find shortest path to any territory owned by the weakest player
    weakest_player_territories = set(game.state.get_territories_owned_by(weakest_player))
    shortest_path = find_shortest_path_from_vertex_to_set(game, most_troops_territory, weakest_player_territories)
    
    # Fortify along the shortest path
